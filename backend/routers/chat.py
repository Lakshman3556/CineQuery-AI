...
async def chat_endpoint(request: ChatRequest):
    ...
    # 3. Perform confidence guardrail check
    ...
    async def chat_stream_generator():
        ...
        # After stream finishes, add the turn to session memory
        full_answer = "".join(accumulated_tokens)
        memory_manager.add_turn(session_id, question, full_answer)
        print(f"[CHAT] Session '{session_id}' updated. Total history size: {len(memory_manager.get_session_memory(session_id).chat_memory.messages) // 2} turns.")
